...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            ...
            for p in self.legalPositions:
                allPossible[p] = 0.0
            allPossible[self.getJailPosition()] = 999.0
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        oldBoard = self.beliefs
        newBoard = util.Counter()

        for p in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for newPos, prob in newPosDist.items():
                newBoard[newPos] += oldBoard[p] * prob

        self.beliefs = newBoard
        #util.raiseNotDefined()
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        if noisyDistance != None:
            weight_table = util.Counter()
            flag = False
            for particle in self.particles:
                trueDistance = util.manhattanDistance(particle, pacmanPosition)
                weight_table[particle] += emissionModel[trueDistance]
                if weight_table[particle] != 0:
                    flag = True
            if not flag:
                self.initializeUniformly(gameState)
            else:
                ...
                for i in range(self.numParticles):
                    self.particles.append(util.sample(weight_table))
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # oldBoard = self.getBeliefDistribution()
        # newBoard = util.Counter()
        # flag = False
        # for p in self.legalPositions:
        #     newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
        ...
        #     self.particles = []
        #     for i in range(self.numParticles):
        #         self.particles.append(util.sample(newBoard))

        oldParticles = self.particles
        self.particles = []

        for i in range(self.numParticles):
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState,
                                                                            oldParticles[i]))
            new_particle = util.sample(newPosDist)
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        count_board = util.Counter()
        for i in range(self.numParticles):
            count_board[self.particles[i]] += 1
        count_board.normalize()
        return count_board
        #util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    ...
                    weight = weight * emissionModels[i][trueDistance]
                else:
                    particle = self.getParticleWithGhostInJail(particle,i)
            weight_table[particle] += weight
            if weight_table[particle] != 0:
                flag = True
        ...
        if not flag:
            ...
            for index in range(self.numParticles):
                for i in range(self.numGhosts):
                    if noisyDistances[i] == None:
                        self.particle[index] = self.getParticleWithGhostInJail(self.particle[index], i)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            latestParticle = newParticle[:]
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(
                    setGhostPositions(gameState, newParticle), i, self.ghostAgents[i]
                )
                latestParticle[i] = util.sample(newPosDist)
                newParticle = latestParticle
            "*** END YOUR CODE HERE ***"
