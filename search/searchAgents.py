...
class CornersProblem(search.SearchProblem):
	...
	def getStartState(self):
		...
		startpoint = (self.startingPosition, Corners)
		return startpoint
	...
	def isGoalState(self, state):
		...
		"*** YOUR CODE HERE ***"
		c = state[1]
		boolean = c[0] and c[1] and c[2] and c[3]
		return boolean
	...
	def getSuccessors(self, state):
		...
		for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
			...
			if not hitsWall:
				...
				if s in self.corners:
					if s == (self.right, 1):
						newCorners = [True, h[1], h[2], h[3]]
					elif s == (self.right, self.top):
						newCorners = [h[0], True, h[2], h[3]]
					elif s == (1, self.top):
						newCorners = [h[0], h[1], True, h[3]]
					elif s == (1,1):
						newCorners = [h[0], h[1], h[2], True]
					successor = ((s, newCorners), action,  1)
				else:
					successor = ((s, h), action, 1)
...
def cornersHeuristic(state, problem):
	...
	while len(node) > 0:
		...
		for i in range(0, len(node)):
			dist = util.manhattanDistance(cp, node[i])
			distArr.append(dist)
		mindist = min(distArr)
		sum += mindist
...
def foodHeuristic(state, problem):
	...
	for i in range(0,foodGrid.width):
		for j in range(0,foodGrid.height):
			if (foodGrid[i][j] == True):
				food_location = (i,j)
				f.append(food_location)  
	if (len(f) == 0):
			return 0		
	max_distance=((0,0),(0,0),0)
	for current_food in f:
		for select_food in f:
	...
	if(max_distance[0]==(0,0) and max_distance[1]==(0,0)):
		h = util.manhattanDistance(p,f[0])
	else: 
		d1 = util.manhattanDistance(p,max_distance[0])
		d2 = util.manhattanDistance(p,max_distance[1])
		h = max_distance[2] + min(d1,d2)
